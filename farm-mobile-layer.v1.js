...
/*! farm-mobile-layer.v1.1.js | UI-only: adds mobile header/bottom bar + transparent glass rail. No event bindings here. */
(function(){
  'use strict';
  ...
  if(!document.getElementById('__mobileLayerCSS__')){
    ...
    #glassInner{
      ...
      box-shadow: 0 0 0 1px rgba(255,255,255,.04) inset; padding:6px 8px calc(64px + env(safe-area-inset-bottom));
    }

    /* Close button top-right (sticky) */
    #glassInner{ position:relative; }
    #glassClose{
      position:sticky; top:6px; left: calc(100% - 36px); z-index:2;
      display:inline-flex; align-items:center; justify-content:center;
      width:28px; height:28px; border-radius:999px; border:1px solid rgba(255,255,255,.22);
      background:rgba(0,0,0,.25); color:#fff;
    }

    /* Mini always centered at top */
    #gMini{ position:sticky; top:6px; z-index:1; display:flex; justify-content:center; padding:4px 2px; }
    #gMini .mini-wrap{
    ...
    /* Gate: make bars slimmer and keep character to the side */
  ...
  // Build glass rail (no events here; router owns it)
  ...
  // Header values (nickname/token/popcorn) refresh only
