...
/*! farm-mobile-layer.v1.js | Adds mobile header/bottombar/transparent glass rail (no server logic touched) */
(function(){
  'use strict';
  var mq = window.matchMedia('(max-width:900px)');
  if(!mq.matches) return; // phones only

  // Hide original fixed bars on mobile
  var st = document.createElement('style');
  st.id = '__mobileLayerCSS__';
  st.textContent = `
  ...
  #glassInner{
    ...
    width:min(560px, 96%); height:100%; overflow-y:auto; overflow-x:hidden; -webkit-overflow-scrolling:touch;
    background:transparent; border:1px solid rgba(255,255,255,.14); border-radius:18px; box-shadow: 0 0 0 1px rgba(255,255,255,.04) inset;
    padding:6px 8px calc(64px + env(safe-area-inset-bottom));
  }
  #glassClose{
    position:sticky; top:6px; margin-left:auto; display:inline-flex; align-items:center; justify-content:center; width:28px; height:28px;
    border-radius:999px; border:1px solid rgba(255,255,255,.22); background:rgba(0,0,0,.25); color:#fff; z-index:2;
  }
  #gMini{ position:sticky; top:6px; z-index:1; display:grid; justify-items:center; padding:4px 2px; }
  #gMini .mini-wrap{ position:static !important; display:grid !important; grid-template-columns:auto 1fr auto; gap:10px; align-items:center; width:100%;
  ...
  var gMini = document.getElementById('gMini');
  var gStage = document.getElementById('gStage');
  var current = null;

  function ensureMini(){
    var mw = document.querySelector('.mini-wrap');
    if(mw && mw.parentElement !== gMini){ gMini.appendChild(mw); }
  }
  function clearStage(){
    var shop=document.getElementById('shop'); if(shop) shop.style.display='none';
  ...
  function openRail(){ document.documentElement.classList.add('glass-on'); }
  function closeRail(){ document.documentElement.classList.remove('glass-on'); current=null; clearStage(); }

  // Close handlers
  document.getElementById('glassClose').addEventListener('click', closeRail);
  document.getElementById('glassBackdrop').addEventListener('click', closeRail);
  document.addEventListener('keydown', function(e){ if(e.key==='Escape'){ closeRail(); } }, {passive:true});

  // Views
  function viewFarm(){
    if(current==='FARM'){ closeRail(); return; }
    clearStage(); ensureMini();
    var L=document.querySelector('.hud-left .panel'); var R=document.querySelector('.hud-right .panel');
    if(L) gStage.appendChild(L); if(R) gStage.appendChild(R);
    current='FARM'; openRail();
  }
  function viewGate(){
    if(current==='GATE'){ closeRail(); return; }
    clearStage(); ensureMini();
    var dock=document.querySelector('.hud-left .gate-dock'); var gate=document.getElementById('gate');
    if(dock) gStage.appendChild(dock);
    if(gate){ gStage.appendChild(gate); gate.style.display='block'; }
    try{ if(typeof updateGateFace==='function') updateGateFace(); }catch(e){}
    current='GATE'; openRail();
  }
  function viewShop(){
    if(current==='SHOP'){ closeRail(); return; }
    clearStage(); ensureMini();
    var shop=document.getElementById('shop'); if(shop){ gStage.appendChild(shop); shop.style.display='block'; }
    current='SHOP'; openRail();
  }

  // Button bridge (works even without router)
  document.getElementById('mBar').addEventListener('click', function(e){
    var btn = e.target.closest('.mbtn'); if(!btn) return;
    var tgt = btn.getAttribute('data-tgt');
    if(tgt==='SYNC'){ if(typeof loadFromServer==='function'){ try{ loadFromServer(); }catch(e){} } return; }
